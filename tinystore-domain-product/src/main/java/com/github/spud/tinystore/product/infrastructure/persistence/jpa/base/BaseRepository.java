 ...
 * 
 * All queries through JPA repositories should use these methods to ensure
 * tenant_id condition is always applied for multi-tenancy data isolation.
 * 
 * Usage pattern:
 * 1. Obtain current tenantId from TenantContext
 * 2. Apply withTenantId specification to all queries
 * 3. Verify at query time that tenantId is not null
 * 
 * Static analysis (SonarQube custom rule) should verify that all
 * JPQL queries include tenant_id in WHERE clause.
 */
public class BaseRepository {
     ...
     */
    public static <T> Specification<T> withTenantIdAnd(String tenantId, Specification<T> additionalSpec) {
