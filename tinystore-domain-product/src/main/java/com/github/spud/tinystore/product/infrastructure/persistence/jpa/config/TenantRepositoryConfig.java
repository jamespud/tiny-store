 ...
 * <p>
 * Provides TenantContextProvider that extracts tenant ID from: 1. HTTP request header X-Tenant-Id
 * (preferred) 2. SecurityContext authenticated principal (fallback) 3. Throws exception if neither
 * available
 * <p>
 * TenantId is stored in request scope for consistent access during a single request lifecycle.
 */
@Configuration
public class TenantRepositoryConfig {
	 ...
	 * <p>
	 * Extraction strategy: 1. X-Tenant-Id header from HttpServletRequest (preferred) 2. tenant_id
	 * claim from JWT in SecurityContext (fallback) 3. Throw IllegalStateException if neither
	 * available
	 */
	@Slf4j
