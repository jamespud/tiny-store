 ...
 * 
 * Detects conflicts before rule activation:
 * 1. Priority conflicts: Rules with same priority in same scope
 * 2. Exclusive group conflicts: Multiple rules in same exclusive group overlap in time
 * 3. Time window overlaps: Rules with incompatible conditions active simultaneously
 * 4. Scope ambiguity: Over-specific rules that shadow broader rules incorrectly
 * 
 * Used during:
 * - Rule creation/update in admin interface
 * - Rule activation workflow
 * - Bulk rule import validation
 * 
...
public class RuleConflictChecker {
     ...
     */
    private List<RuleConflict> checkPriorityConflicts(PricingRule newRule, List<PricingRule> existingRules) {
     ...
     * Rules in same exclusive group should not have overlapping time windows
     * 
     * @param newRule Rule to check
     * @param existingRules Existing rules
     * @return List of exclusive group conflicts
     */
    private List<RuleConflict> checkExclusiveGroupConflicts(PricingRule newRule, List<PricingRule> existingRules) {
     ...
     */
    private List<RuleConflict> checkTimeWindowConflicts(PricingRule newRule, List<PricingRule> existingRules) {
    ...
    public static class RuleConflict {
        ...
        public String getRuleCode1() { return ruleCode1; }
        public String getRuleCode2() { return ruleCode2; }
