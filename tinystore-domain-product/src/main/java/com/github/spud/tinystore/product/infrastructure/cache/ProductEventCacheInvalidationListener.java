 ...
 * 
 * Listens to domain events and invalidates corresponding cache entries:
 * - ProductCreatedEvent, ProductUpdatedEvent -> Invalidate product:{tenantId}:{productId}
 * - SkuCreatedEvent, SkuUpdatedEvent -> Invalidate sku:{tenantId}:{skuId}
 * - PricingRuleChangedEvent -> Invalidate pricing:rules:{tenantId}:{productId}
 * 
 * Cache invalidation strategy:
 * 1. Event-driven (immediate invalidation on domain changes)
 * 2. TTL-based expiration (fallback for missed events)
 * 
...
public class ProductEventCacheInvalidationListener {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ProductEventCacheInvalidationListener.class);
