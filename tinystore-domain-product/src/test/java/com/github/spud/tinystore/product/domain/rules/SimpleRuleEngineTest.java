...
public class SimpleRuleEngineTest {
	...
	void percent_and_flat_rules_apply_with_priority_and_group() {
		var sku = Sku.create("sku-1", "p-1", new SpecificationCombination(), "BAR123");
		sku.setBasePrice(Money.of(new BigDecimal("100.00")));
		var ctx = new PricingContext(sku, Map.of("brand", "nike"), Instant.now());

		var percent10 = new TimeWindowPercentOffRule("TW10", 100, "promo",
			Instant.now().minusSeconds(60), Instant.now().plusSeconds(60), new BigDecimal("0.10"));
		var flat20Nike = new TagFlatOffRule("BRAND20", 90, null, "brand", "nike",
