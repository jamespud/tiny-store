...
public class CheckoutAppService {
    ...
    private CheckoutQuoteResponse createQuote(CheckoutQuoteRequest request) {
        ...
        //仅筛选可用优惠券，不锁定
        CouponPlanResult couponPlan = selectAvailableCoupons(quoteId, request, snapshot, now);
    ...
    // 仅筛选可用优惠券，不锁定
    private CouponPlanResult selectAvailableCoupons(UUID quoteId, CheckoutQuoteRequest request, PricingSnapshot snapshot,
                                                    LocalDateTime now) {
        ...
        if (!invalidCoupons.isEmpty()) {
            ...
            return new CouponPlanResult(List.of(), List.of(), invalidReasons);
        }
        ...
        for (ScopedCouponInput input : inputs) {
            ...
            // 校验用户是否有可用的该优惠券（未使用、未过期）
            Optional<UserCouponEntity> userCouponOpt = userCouponRepository
                    .findFirstByUserIdAndCouponIdAndUseStatusOrderByReceiveTimeAsc(request.getUserId(), c.getId(), "UNUSED");
            if (userCouponOpt.isEmpty()) {
                invalidReasons.add(ChangeReason.of("COUPON_UNAVAILABLE", c.getCouponNo() + ":NO_UNUSED"));
                continue;
            }
            ...
            if (discount <= 0L) {
                invalidReasons.add(ChangeReason.of("COUPON_UNAVAILABLE", c.getCouponNo() + ":DISCOUNT_ZERO"));
                continue;
            }
        ...
        for (Map.Entry<String, List<CouponCandidate>> entry : candidatesByGroup.entrySet()) {
            ...
            // 仅记录候选优惠券，不锁定
            CouponCandidate cand = candidates.get(0);
            ...
            b.setBenefitId(cand.coupon.getCouponNo()); // 存储couponNo，便于后续扣减
            b.setGroupKey(group);
    ...
    private boolean lockCouponsAtomic(CheckoutQuotePayload payload, String tradeId, LocalDateTime lockExpireTime,
        ...
        for (PricingSnapshot.AppliedBenefit b : benefits) {
            if (!"PLATFORM_COUPON".equals(b.getBenefitType()) && !"SHOP_COUPON".equals(b.getBenefitType())) {
                continue;
            }

            String couponNo = b.getBenefitId();
            if (couponNo == null || couponNo.isBlank()) {
                changes.add(ChangeReason.of("COUPON_INVALID", "EMPTY_COUPON_NO"));
                return false;
            }

            // 1. 查询优惠券信息
            Optional<CouponEntity> couponOpt = couponRepository.findByCouponNo(couponNo);
            if (couponOpt.isEmpty()) {
                changes.add(ChangeReason.of("COUPON_NOT_FOUND", couponNo));
                return false;
            }
            ...
            // 2. 行锁查询用户优惠券（保证原子性）
            Optional<UserCouponEntity> userCouponOpt = userCouponRepository
                    .findFirstByUserIdAndCouponIdAndUseStatusForUpdate(
                            payload.getQuoteRequest().getUserId(),
                            coupon.getId(),
                            "UNUSED"
                    );
            if (userCouponOpt.isEmpty()) {
    ...
    private record CouponCandidate(CouponEntity coupon, long discountCents) {
    }
