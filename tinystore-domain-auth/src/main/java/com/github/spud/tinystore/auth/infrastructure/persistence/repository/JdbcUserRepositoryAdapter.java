...
public class JdbcUserRepositoryAdapter implements UserRepository {
  ...
  public MallUser update(MallUser user) {
    jdbcTemplate.update(
        "update mall_user set phone = ?, nickname = ?, avatar = ?, status = ?, rt_version = ?, password_hash = ?, updated_at = now() where id = ?",
        user.getPhone().value(),
        user.getUsername(),
        user.getAvatar(),
  ...
  private static MallUserStatus mapStatus(String status) {
    ...
    return "FROZEN".equalsIgnoreCase(status) || "frozen".equalsIgnoreCase(status)
        ? MallUserStatus.FROZEN
        : MallUserStatus.ACTIVE;
  }

  private static String toColumnStatus(MallUserStatus status) {
    return status == MallUserStatus.FROZEN ? "FROZEN" : "ACTIVE";
  }
}