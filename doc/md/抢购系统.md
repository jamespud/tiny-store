...
sequenceDiagram
    ...
    alt 通过限流
        ...
        alt 预扣成功
            ...
            loop 创建订单
            StockService ->> Database: 修改库存
            end
            alt 创建成功
                Database -->> StockService: 修改结果
%%                StockService -->> MQ: 发送结果反馈消息
%%                MQ -->> OrderService: 订单处理结果
%%                OrderService -->> User: 通知支付
                alt 支付成功
...
%%            else 创建失败
%%                StockService ->> StockService: 重试
%%                StockService -->> MQ: 发送失败反馈
%%                MQ -->> OrderService: 订单失败
%%                OrderService ->> StockService: 触发库存回退
%%                StockService -->> Redis: 回退库存
%%                OrderService -->> User: 返回失败结果
            end
        else 预扣失败
            Gateway -->> User: 返回“已售罄”
        end
