...
- 当前阶段仅在订单定价流程中通过 `PromotionClient` 进行优惠券预占（pre-use），支付成功回调不再直接触发优惠券核销确认。
- 未来若需要在支付回调中完成确认，将在业务方案评审后引入：
   - 独立的配置开关（如 `promotion.confirm-on-payment.enabled`）以控制启用；
   - 明确的入参契约（包括锁定 ID、券明细等字段），避免依赖 `extraData` 临时参数；
   - 失败补偿与重试机制，并提供监控与告警；
   - 与订单状态机、Outbox 事件时序的配套调整。
- 在促销服务具备完备的幂等、补偿能力之前，推荐由后续异步流程或专门任务触发确认核销，保证主流程稳定。
