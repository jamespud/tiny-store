...
public class OrderDomainService {
	...
	public OrderItem loadForUpdate(String orderId) {
		...
		if (mainOrderNo == null) {
			throw new OrderBusinessException("Order not found: " + orderId, "ORDER-4040", HttpStatus.NOT_FOUND);
		}
		OrderMainEntity main = orderMainJpaRepository.findForUpdateByOrderNo(mainOrderNo)
			.orElseThrow(() -> new OrderBusinessException("Order not found: " + mainOrderNo, "ORDER-4040", HttpStatus.NOT_FOUND));
		List<OrderItemEntity> items = orderItemJpaRepository.findByMainOrderNoOrderByLineNoAsc(mainOrderNo);
		...
		agg.setLines(lines);
		agg.setTotal(Money.ofCents(main.getTotalAmount() == null ? 0L : main.getTotalAmount(), main.getCurrency()));
		agg.setPayable(Money.ofCents(main.getPayableAmount() == null ? 0L : main.getPayableAmount(), main.getCurrency()));
	...
	public void save(OrderItem order, long expectedVersion) {
		...
		OrderMainEntity main = orderMainJpaRepository.findByOrderNo(order.getOrderNo())
			.orElseThrow(() -> new OrderBusinessException("Order not found: " + order.getOrderNo(), "ORDER-4040", HttpStatus.NOT_FOUND));
	...
	private LineItem mapToLineItem(OrderItemEntity it) {
		...
		if (it.getSkuSnapshot() != null && !it.getSkuSnapshot().isBlank()) {
			try {
				...
				String currency = m.get("currency") != null ? String.valueOf(m.get("currency")) : it.getCurrency();
				long unitPriceCents = 0L;
				Object up = m.get("unitPriceCents");
				if (up instanceof Number n) {
					unitPriceCents = n.longValue();
				} else if (up != null) {
					try { unitPriceCents = Long.parseLong(String.valueOf(up)); } catch (Exception ignored) { }
				}
				Instant snapshotTime = Instant.now();
				Object st = m.get("snapshotTime");
				if (st != null) {
					try { snapshotTime = Instant.parse(String.valueOf(st)); } catch (Exception ignored) { }
				}
		...
		Money unit = Money.ofCents(it.getUnitPriceAmount() == null ? 0L : it.getUnitPriceAmount(), it.getCurrency());
		Money total = Money.ofCents(it.getLineTotalAmount() == null ? 0L : it.getLineTotalAmount(), it.getCurrency());
		Money discount = Money.ofCents(it.getLineDiscountAmount() == null ? 0L : it.getLineDiscountAmount(), it.getCurrency());
		Money payable = Money.ofCents(it.getLinePayableAmount() == null ? 0L : it.getLinePayableAmount(), it.getCurrency());
