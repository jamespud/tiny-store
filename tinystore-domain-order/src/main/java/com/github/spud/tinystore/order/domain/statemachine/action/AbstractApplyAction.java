...
public abstract class AbstractApplyAction implements Action<CoreFlowStatus, OrderEvent> {
  ...
  public void execute(StateContext<CoreFlowStatus, OrderEvent> context) {
    ...
    try {
      var envelope = outboxEventService.buildEvent(eventType, orderId, null, tenantId, actorId,
        domainData);
      log.debug("StateMachine action executed: type={}, orderId={}, envelopeKeys={}", eventType,
        orderId, envelope.keySet());
      recordAudit(orderId, actorId, true, null);
    } catch (Exception e) {
      log.warn("StateMachine action failed: type={}, orderId={}, error={}", eventType, orderId,
        e.getMessage());
  ...
  private void recordAudit(String orderId, String actorId, boolean ok, String errorCode) {
    ...
    String tenantId = MDC.get("tenantId");
    AuditEntry entry = new AuditEntry(System.currentTimeMillis(), eventType, orderId, actorId,
      tenantId, traceId, ok, errorCode);
    try {
